import { addEventListeners, css, on, ref } from "/assets/pkg/remix/src/ui.js";
import { dragVelocityEvents } from "/assets/demos/ui/animation/demos/drag-release.js";
...
function PointerTrail(handle) {
	...
	function draw() {
		...
		for (let i = 1; i < points.length; i++) {
			...
			ctx.strokeStyle = `rgba(14, 165, 233, ${segmentOpacity * .6})`;
		...
		if (points.length > 0) {
			...
			gradient.addColorStop(0, `rgba(239, 68, 68, ${overallOpacity * .6})`);
	...
	return () => /* @__PURE__ */ _jsx("canvas", { mix: [ref((node) => {
		canvas = node;
...
export default function SpringDemo(handle) {
	let viewportWidth = globalThis.window?.innerWidth ?? 1e3;
	...
	if (typeof document !== "undefined") {
		addEventListeners(document, handle.signal, { click(event) {
	...
	return () => /* @__PURE__ */ _jsxs("div", {
		...
		children: [
			...
			/* @__PURE__ */ _jsx("div", {
				...
				children: Object.keys(spring.presets).map((preset) => /* @__PURE__ */ _jsxs("label", {
					mix: [css({
						display: "flex",
						alignItems: "center",
						gap: "6px",
						...
						"&:has(input:checked)": { backgroundColor: "rgba(14, 165, 233, 0.3)" }
					})],
					children: [/* @__PURE__ */ _jsx("input", {
						...
						mix: [css({ accentColor: "#0ea5e9" }), on("change", () => {
