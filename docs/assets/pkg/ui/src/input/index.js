...
const rootBaseStyle = css({
	...
	"&:focus-within": { boxShadow: controlFocusShadow },
	"&:has(input:disabled), &:has(input[aria-disabled=\"true\"])": { opacity: .55 },
	"& > svg": {
		flex: "none",
...
const fieldBaseStyle = css({
	...
	"&:disabled, &[aria-disabled=\"true\"]": { opacity: 1 }
});
const sizeStyles = {
	md: css({}),
	lg: css({
...
export const input = Object.assign(createInput, {
	root,
	field
});
