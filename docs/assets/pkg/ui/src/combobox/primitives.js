...
import * as listbox from "/assets/pkg/ui/src/listbox/index.js";
...
function ComboboxProvider(handle) {
	...
	function resolveOpenOption(strategy, nextFilterText) {
		...
		if (strategy === "selected-or-none") {
			...
			if (selectedOption && !selectedOption.disabled && matchesSearchValue(selectedOption.textValue ?? selectedOption.label, nextFilterText)) {
		...
		if (strategy === "selected") {
			...
			if (exactInputMatch && matchesSearchValue(exactInputMatch.textValue ?? exactInputMatch.label, nextFilterText)) {
		...
		return strategy === "last" ? enabledMatchingOptions[enabledMatchingOptions.length - 1] : enabledMatchingOptions[0];
	}
	...
	async function closePopover() {
		...
		if (pendingInputValue !== null) {
			...
			if (signal.aborted || closeId !== closeSequenceId || open || pendingInputValue !== nextValue) {
				return;
			}
	...
	return () => {
		...
		return /* @__PURE__ */ _jsx(popover.Context, { children: /* @__PURE__ */ _jsx(listbox.Context, {
			activeValue,
			flashSelection: true,
