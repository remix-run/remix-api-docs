...
export class AccordionChangeEvent extends Event {
	...
	constructor(value, init) {
		super(ACCORDION_CHANGE_EVENT, { bubbles: true });
...
const accordionItemCss = css({ minWidth: 0 });
const accordionTriggerCss = css({
	...
	textAlign: "left",
	"&:hover:not(:disabled)": { backgroundColor: theme.surface.lvl1 },
	"&:focus-visible": {
...
function AccordionImpl(handle) {
	...
	let toggleItem = (itemValue) => {
		...
		if (isMultipleProps(handle.props)) {
			...
			let nextValue = currentValue.includes(itemValue) ? currentValue.filter((value) => value !== itemValue) : [...currentValue, itemValue];
			if (handle.props.value === undefined) {
	...
	let focusItem = (itemValue, direction) => {
		...
		switch (direction) {
			...
			case "previous":
				...
				break;
			case "next":
				targetIndex = currentIndex === -1 || currentIndex === items.length - 1 ? 0 : currentIndex + 1;
				break;
		}
...
export function AccordionTrigger(handle) {
	return () => {
		...
		let button = /* @__PURE__ */ _jsxs("button", {
			...buttonProps,
			...
			type: type ?? "button",
			children: [/* @__PURE__ */ _jsx("span", { children }), indicator === null ? null : /* @__PURE__ */ _jsx("span", {
				"data-rmx-accordion-indicator": "",
				"data-state": item.open ? "open" : "closed",
				mix: indicatorStyle,
				children: indicator ?? /* @__PURE__ */ _jsx(Glyph, { name: "chevronRight" })
			})]
		});
