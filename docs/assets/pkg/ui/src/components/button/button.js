	...
	return () => {
		...
		return /* @__PURE__ */ _jsxs("button", {
			...
			children: [
				...
				children !== undefined ? /* @__PURE__ */ _jsx("span", {
					mix: labelStyle,
					children
				}) : null,
...
function createButtonCss(buttonTheme) {
	...
	return css({
		...
		"&:active": {
			...
			boxShadow: `${createButtonHighlight(buttonTheme.backgroundActive, .12)}, ${theme.shadow.xs}`
		},
		"&:focus-visible": {
			outline: `2px solid ${theme.colors.focus.ring}`,
			outlineOffset: "2px"
...
function createButtonBackgroundImage(background) {
	return `linear-gradient(to bottom, ${createButtonHighlightColor(background, .18)} 0%, ${background} 100%)`;
}
function createButtonBorderColor(color) {
	return `color-mix(in oklab, ${color} 74%, black)`;
}
function createButtonHighlight(background, amount = .16) {
