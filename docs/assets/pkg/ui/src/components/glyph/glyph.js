...
export function createGlyphSheet(values) {
	...
	function GlyphSheet(handle) {
		return () => {
			...
			let nextStyle = typeof style === "object" && style !== null ? {
				...hiddenStyle,
				...style
			} : hiddenStyle;
...
export function Glyph(handle) {
	return () => {
		...
		let hiddenByDefault = handle.props["aria-hidden"] === undefined && handle.props["aria-label"] === undefined && handle.props["aria-labelledby"] === undefined;
		return createElement("svg", {
			...svgProps,
			"aria-hidden": hiddenByDefault ? true : handle.props["aria-hidden"],
			fill: fill ?? "none",
			xmlns: "http://www.w3.org/2000/svg"
		}, createElement("use", { xlinkHref: `#${glyphId}` }));
	};
}
