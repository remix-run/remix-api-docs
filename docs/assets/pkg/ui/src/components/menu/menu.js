import { attrs, createMixin, css, on, ref } from "/assets/pkg/ui/src/index.js";
import * as button from "/assets/pkg/ui/src/components/button/button.js";
import { Glyph } from "/assets/pkg/ui/src/components/glyph/glyph.js";
import * as popover from "/assets/pkg/ui/src/components/popover/popover.js";
import { theme } from "/assets/pkg/ui/src/theme/theme.js";
import { hiddenTypeahead, matchNextItemBySearchText } from "/assets/pkg/ui/src/interactions/typeahead/typeahead-mixin.js";
import { waitForCssTransition } from "/assets/pkg/ui/src/utils/wait-for-css-transition.js";
import { wait } from "/assets/pkg/ui/src/utils/wait.js";
import { createHoverAim } from "/assets/pkg/ui/src/components/menu/hover-aim.js";
...
const menuPopoverCss = css({
	"&[data-menu-submenu=\"true\"][data-anchor-placement^=\"right\"]": { marginLeft: `calc(${theme.space.xs} * -1)` },
	"&[data-menu-submenu=\"true\"][data-anchor-placement^=\"left\"]": { marginLeft: theme.space.xs },
...
const menuItemCss = css({
	...
	"&:focus": { outline: "none" },
	"&[data-highlighted=\"true\"]": { color: theme.colors.action.primary.foreground },
	"&[data-highlighted=\"true\"]::before": { backgroundColor: theme.colors.action.primary.background },
	"&[aria-haspopup=\"menu\"][aria-expanded=\"true\"]:not(:focus)": { color: theme.colors.text.primary },
	"&[aria-haspopup=\"menu\"][aria-expanded=\"true\"]:not(:focus)::before": { backgroundColor: theme.surface.lvl2 },
	"&[data-submenu-state=\"selecting\"], &[data-submenu-state=\"dismissing\"]": { color: theme.colors.text.primary },
	"&[data-submenu-state=\"selecting\"]::before, &[data-submenu-state=\"dismissing\"]::before": { backgroundColor: theme.surface.lvl2 },
	"&[data-menu-flash=\"true\"]": { color: theme.colors.text.primary },
	"&[data-menu-flash=\"true\"]::before": { backgroundColor: "transparent" },
	"&[aria-disabled=\"true\"]": { opacity: .5 },
	"&[aria-checked=\"true\"], &[aria-selected=\"true\"]": { "--rmx-menu-item-indicator-opacity": "1" }
});
...
function MenuProvider(handle) {
	...
	function navigate(strategy) {
		...
		switch (strategy) {
			case "next":
				nextItem = activeIndex === -1 ? interactableItems[0] : interactableItems[activeIndex + 1] ?? interactableItems[activeIndex];
				break;
			case "previous":
				nextItem = activeIndex === -1 ? interactableItems[interactableItems.length - 1] : interactableItems[activeIndex - 1];
				break;
			case "first":
	...
	async function activateItem(id) {
		...
		state = "selecting";
		activeId = item.id;
		flashState = item.type === "item" ? { id: item.id } : {
	...
	return () => {
		...
		return /* @__PURE__ */ _jsx(popover.Context, { children: handle.props.children });
	};
}
	...
	return () => [
		...
		popover.surface({
			...
			onHide(request) {
				...
				if (request?.reason === "outside-click" && request.target instanceof Node && context.root.surfaceNode?.contains(request.target)) {
...
const itemMixin = createMixin((handle) => {
	...
	return (options) => {
		...
		return [attrs({
			...
			tabIndex: -1,
			"aria-checked": type === "item" ? undefined : isFlashing ? context.flashingChecked ? "true" : "false" : options.checked ? "true" : "false",
		...
		return [
			...
			!options.disabled && [
				...
				on("pointerleave", (event) => {
					...
					if (childMenu.isOpen && parent.startHoverAim(itemRef ?? null, childSurface ?? null, event)) {
						return;
					}
				...
				on("focus", (event) => {
					...
					if (event.relatedTarget instanceof Element && event.relatedTarget.closest("[role=\"menu\"]")?.id === childMenu.listId) {
						return;
					}
					scheduleOpen();
				}),
