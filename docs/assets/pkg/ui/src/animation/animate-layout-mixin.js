...
function calcBoxDelta(delta, source, target, layoutConfig) {
	let origin = layoutConfig.size === false ? 0 : .5;
...
function isTargetBoxSame(source, target, layoutConfig) {
	return isNear(source.x.min, target.x.min, TRANSLATE_PRECISION) && isNear(source.y.min, target.y.min, TRANSLATE_PRECISION) && (layoutConfig.size === false || isNear(source.x.max, target.x.max, TRANSLATE_PRECISION) && isNear(source.y.max, target.y.max, TRANSLATE_PRECISION));
}
