import { createElement, createMixin, on, ref } from "/assets/pkg/ui/src/index.js";
...
const controlMixin = createMixin((handle, hostType) => {
	...
	function getChecked(options, props) {
		...
		if (!hasInitialized) {
			uncontrolledChecked = getBooleanState(options.defaultChecked) ?? getBooleanState(props.defaultChecked) ?? false;
			hasInitialized = true;
		}
	...
	return (options = {}, props) => {
		...
		let nextProps = {
			...
			role: "switch",
			tabIndex: hostType === "input" ? options.tabIndex ?? props.tabIndex : disabled ? undefined : options.tabIndex ?? props.tabIndex ?? 0,
			type: hostType === "input" ? props.type === undefined ? "checkbox" : props.type : hostType === "button" && props.type === undefined ? "button" : props.type,
		...
		return createElement(handle.element, {
			...
			mix: [
				...
				on("click", (event) => {
					...
					let nextChecked = event.currentTarget instanceof HTMLInputElement ? event.currentTarget.checked : !checked;
