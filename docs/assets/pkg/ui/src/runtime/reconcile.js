import { createComponent, Frame } from "/assets/pkg/ui/src/runtime/component.js";
import { createFrame } from "/assets/pkg/ui/src/runtime/frame.js";
import { createRangeRoot } from "/assets/pkg/ui/src/runtime/vdom.js";
import { isCommittedComponentNode, isComponentNode, isCommittedHostNode, isCommittedTextNode, isFragmentNode, isHostNode, isNonRenderNode, isTextNode, findContextFromAncestry } from "/assets/pkg/ui/src/runtime/vnode.js";
import { invariant } from "/assets/pkg/ui/src/runtime/invariant.js";
import { patchHostProps } from "/assets/pkg/ui/src/runtime/core/props.js";
import { skipComments, logHydrationMismatch } from "/assets/pkg/ui/src/runtime/client-entries.js";
import { toVNode } from "/assets/pkg/ui/src/runtime/to-vnode.js";
import { bindMixinRuntime, cancelPendingMixinRemoval, dispatchMixinBeforeUpdate, dispatchMixinCommit, getMixinRuntimeSignal, prepareMixinRemoval, resolveMixedProps, teardownMixins } from "/assets/pkg/ui/src/runtime/mixins/mixin.js";
import { isOnMixinDescriptor } from "/assets/pkg/ui/src/runtime/mixins/on-mixin.js";
...
export function remove(node, domParent, scheduler, styles) {
	...
	if (isCommittedHostNode(node)) {
		...
		if (persistedRemoval) {
			...
			void persistedRemoval.catch(() => {}).finally(() => {
				if (!node._persistedByMixins) return;
...
function diffChildren(curr, next, domParent, frame, scheduler, styles, vParent, rootTarget, cursor, anchor) {
	...
	if (next.length === 0 && anchor === undefined && !parentUsesInnerHTML(vParent) && canBulkClearChildren(curr)) {
...
function reclaimPersistedMixinNode(persistedNode, newNode, frame, scheduler, styles, vParent, rootTarget) {
	...
	if (shouldDispatchInlineMixinLifecycle(persistedNode._dom)) {
		scheduler.enqueueCommitPhase([() => dispatchMixinCommit(newNode._mixState)]);
	}
}