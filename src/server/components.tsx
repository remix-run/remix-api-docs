...
export function Nav(handle: Handle) {
  let { docFiles } = handle.context.get(App)
  return () => {
...
function NavDropdownSection(handle: Handle) {
  let { activeVersion: version, slug } = handle.context.get(App)
      ...
      <>
        ...
        {...files[type].map((file) => (
          <li>
            ...
            <a
              href={routes.api.href({ version, slug: file.urlPath })}
              class={file.urlPath === slug ? 'active' : undefined}
            >
              {file.name}
            </a>
          </li>
