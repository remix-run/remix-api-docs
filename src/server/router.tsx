...
const MD_DIR = path.join('build', 'md')
const PUBLIC_DIR = path.join('public')
const REMIX_PKG_JSON = path.join('..', 'remix', 'packages', 'remix', 'package.json')
...
export function createRouter(versions?: AppContext['versions']) {
  versions ??= [
    {
      version: JSON.parse(fs.readFileSync(REMIX_PKG_JSON, 'utf-8')).version,
      crawl: true,
    },
  ]
